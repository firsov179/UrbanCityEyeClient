...
class HomeView:
    ...
    def _update_start_button(self):
        ...
        if self.selected_city_id == 1:
            self.start_btn.disabled = False
            self.start_btn.style.display = "block"
            
            coming_soon_element = js.document.getElementById("coming-soon-message")
            if coming_soon_element:
                coming_soon_element.remove()
        else:
            self.start_btn.style.display = "none"
            
            current_lang = js.document.documentElement.lang or "en"
            is_russian = current_lang == "ru"
            coming_soon_message = "Моделирование для этого города будет добавлено позже." if is_russian else "Simulation for this city will be added later."
            
            existing_element = js.document.getElementById("coming-soon-message")
            if not existing_element:
                coming_soon_element = js.document.createElement("div")
                coming_soon_element.id = "coming-soon-message"
                coming_soon_element.className = "coming-soon-message"
                coming_soon_element.innerHTML = f"<p>{coming_soon_message}</p>"
                
                self.start_btn.parentNode.insertBefore(coming_soon_element, self.start_btn.nextSibling)
            else:
                existing_element.innerHTML = f"<p>{coming_soon_message}</p>"
